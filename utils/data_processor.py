...
def process_file(file):
    ...
    slow_signal_popt = sig_fitter.fit_slow_component(new_time, new_signals, right, fast_signal_popt)

    rise_time = -1*fast_signal_popt[2]*np.log((1-0.9)/0.9*0.1/(1-0.1))
    return {'trigger': trigger_popt, 'signal': [*(fast_signal_popt[:4]), *slow_signal_popt, fast_signal_popt[4]], 'rise_time': [rise_time]}
